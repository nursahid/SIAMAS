...
class Users extends MY_Controller {
	...
	public function index() {
        ...
        if ($this->uri->segment(3) !== 'read') {
            ...
            if ($this->ion_auth->in_group($admin_group)) {
                $crud->edit_fields('username', 'full_name', 'email', 'phone', 'photo', 'groups', 'last_login', 'old_password', 'new_password');
                $crud->set_relation_n_n('groups', 'users_groups', 'groups', 'user_id', 'group_id', 'name');
            } else {
                //$crud->edit_fields('username', 'full_name', 'email', 'phone', 'photo', 'last_login', 'old_password', 'new_password');
				$crud->edit_fields('username', 'full_name', 'email', 'phone', 'photo', 'last_login');
            }
        } else {
		...
		$crud->display_as("photo", "Foto Profil");
		$crud->display_as("phone", "No. Telepon/HP");	
		$crud->display_as("old_password", "Password Lama");
		$crud->display_as("new_password", "Password Baru");	
		$crud->display_as("last_login", "Login Terakhir");	

        // Validation
        //$crud->required_fields('username', 'full_name', 'email', 'password', 'password_confirm');
		$crud->required_fields('username', 'full_name', 'email');
		...
		$template_data["crumb"] = ["Manajemen Pengguna" => "admin/users/index/edit/".$this->uri->segment(5)."",];
	...
	public function password($user_id) {
        ...
        if ($this->uri->segment(3) !== 'read') {
            $crud->add_fields('username', 'groups', 'password', 'password_confirm');
            $admin_group = $this->config->item('admin_group', 'ion_auth');
            if ($this->ion_auth->in_group($admin_group)) {
                $crud->edit_fields('username', 'last_login', 'old_password', 'new_password');
                $crud->set_relation_n_n('groups', 'users_groups', 'groups', 'user_id', 'group_id', 'name');
            } else {
                $crud->edit_fields('username', 'last_login', 'old_password', 'new_password');
            }
        ...
        // Validation
        //$crud->required_fields('username', 'password', 'password_confirm');
		$crud->required_fields('old_password', 'new_password');
        ...
        if ($this->uri->segment(3) == 'password') {
            $crud->unset_back_to_list();
			$crud->set_lang_string('update_success_message', 'Data berhasil disimpan<br/> Mohon tunggu dialihkan ke halaman berikutnya...
				<script type="text/javascript">
				window.location = "'.site_url('admin/profile').'";
		...
		$template_data["title"] = "Manajemen Pengguna";
		$template_data["crumb"] = ["Manajemen Pengguna" => "#","Ganti Password" => "admin/users/password/edit/".$this->uri->segment(5)."",];
    ...
    public function active_callback($value, $row)
    {
        ...
        return "<a href='".site_url('admin/users/manual_activate/'.$row->id.'/'.$value)."'>$val</a>";
    }
