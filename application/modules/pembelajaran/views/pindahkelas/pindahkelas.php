...
<div class="box box-primary">
	<div class="box-header with-border">
		<h3 class="box-title"><i class="fa fa-table"></i> Pindah Kelas Tahun Pelajaran : <strong><?=$this->sistem_model->get_nama_tapel('Y');?></strong></h3>
			...
			<tbody>
				<tr>
					<td align="center">
						...
						<div class="form-group">
							...
							<div class="col-sm-6"> 
								<input type="text" name="tahunajaran" value="<?=@$thnsekarang->tahun?>" id="tahunajaran" class="form-control" readonly="true" />
					...
					<td align="center">
						<input type="button" name="to2" id="to2" class="btn btn-danger btn-xs" title='assign' style="font-size: 18px;" value="&nbsp;&nbsp;&nbsp;&#8608&nbsp;&nbsp;" /><br/><br/>
					</td>
					<td align="center">
						...
						<div class="form-group">
							<label for="tahunajaranawal" class="col-sm-4 control-label">Tahun Ajaran <span class="required-input">*</span></label>
							<div class="col-sm-6"> 
								<input type="text" name="tahunajaranakhir" value="<?=@$thnsekarang->tahun?>" id="ti" class="form-control" readonly="true" />
								<input type="hidden" name="to" value="<?=@$thnsekarang->id?>" id="tahunajaranakhir" />
							</div>
						</div>
						<div class="form-group">
							<label for="kelas" class="col-sm-4 control-label">Kelas Baru <span class="required-input">*</span></label>
							<div class="col-sm-6"> 
				...
				<tr>
					...
					<td class="table-common-links"><br/>
						<input type="hidden" name="id_siswa" id="nis" value=""><a class="btn btn-sm btn-primary" href="javascript: return void(0)" id="add"><i class="fa fa-plus"></i> PINDAH KELAS</a>
					</td>
				</tr>
			</tbody>
		</table>
...
$(function(){
	...
	$('#awal').change(function() {		
		var selectValues = $("#awal").val();
			if (selectValues == 0){				
				...
				$.ajax({
						type: "POST",
						url : '<?=base_url()?>pembelajaran/pindahkelas/pilihkelas',
						data: mp,
		...
		$('#renderkelasawal').load('<?=base_url()?>pembelajaran/pindahkelas/loadkelas/' + $('#awal').val() + '/' + $('#ta').val());
	});	
	...
	$('#mp_id').change(function() {		
		...
		if (selectValues == 0){				
			...
			$('#renderkelasakhir').load('<?=base_url()?>pembelajaran/pindahkelas/loadkelasakhir/' + $('#mp_id').val() + '/' + $('#tahunajaranakhir').val());
		}		
	});	
