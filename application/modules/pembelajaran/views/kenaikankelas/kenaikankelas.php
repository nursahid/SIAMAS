...
<div class="box box-primary">
	<div class="box-header with-border">
		<h3 class="box-title"><i class="fa fa-table"></i> Kenaikan Kelas Tahun Pelajaran : <strong><?=$this->sistem_model->get_nama_tapel('Y');?></strong></h3>
			...
			<tbody>
				<tr>
					<td align="center">
						...
						<div class="form-group">
							<label for="tahunajaranawal" class="col-sm-4 control-label">Tapel Awal <span class="required-input">*</span></label>
							<div class="col-sm-6"> 
								<input type="text" name="tahunajaran" value="<?=@$thnsebelum->tahun?>" id="tahunajaran" class="form-control" readonly="true" />
								<input type="hidden" name="ta" value="<?=@$thnsebelum->id?>" id="ta" />
							</div>
						</div>
						<div class="form-group">
							<label for="kelas" class="col-sm-4 control-label">Kelas Lalu <span class="required-input">*</span></label>
							<div class="col-sm-6"> 
								<?php
								echo form_dropdown('kelas', $kenaikankelas, '0','class="form-control input-sm required" id="awal"');             
								?>
							</div>
						</div>
						<div class="form-group">
					...
					<td align="center">
						<br/><br/>
						<input id="to2" type="button" name="to2" class="btn btn-danger btn-xs" title='assign' style="font-size: 18px;" value="&nbsp;&nbsp;&nbsp;&#8608&nbsp;&nbsp;" />
						<br/><br/>						
					</td>
					<td align="center">
						<div class="form-group">
							<label for="tahunajaranawal" class="col-sm-4 control-label">Tapel Sekarang <span class="required-input">*</span></label>
							<div class="col-sm-6"> 
								<input type="text" name="tahunajaranakhir" value="<?=@$thnsesudah->tahun?>" id="ti" class="form-control" readonly="true" />
								<input type="hidden" name="to" value="<?=@$thnsesudah->id?>" id="tahunajaranakhir" />
							</div>
						</div>
						<div class="form-group">
							<label for="kelas" class="col-sm-4 control-label">Kelas Sekarang <span class="required-input">*</span></label>
							<div class="col-sm-6"> 
								<div id="mapel"><?=form_dropdown("kelasakhir",array('Pilih Kelas'=>'-- Pilih Kelas --'),'','class="form-control input-sm required" disabled');?></div>
							</div>
						</div>
						<div class="form-group">
							<label for="db_idkelas" class="col-sm-4 control-label">Data Siswa <span class="required-input">*</span></label>
				...
				<tr>
					...
					<td class="table-common-links"><br/>
						<input type="hidden" name="id_siswa" id="nis" value=""><a class="btn btn-sm btn-primary" href="javascript: return void(0)" id="add"><i class="fa fa-plus"></i> NAIK KELAS </a>
					</td>
				</tr>
			</tbody>
		</table>
...
$(function(){
	...
	$('#awal').change(function() {	
		var selectValues = $("#awal").val();
			if (selectValues == 0){				
				...
				$.ajax({
						type: "POST",
						url : '<?=base_url()?>pembelajaran/kenaikankelas/pilihkelas',
						data: mp,
		...
		$('#renderkelasawal').load('<?=base_url()?>pembelajaran/kenaikankelas/loadkelas/' + $('#awal').val() + '/' + $('#ta').val());
	});	
	...
	$('#mp_id').change(function() {		
		...
		if (selectValues == 0){				
			...
			$('#renderkelasakhir').load('<?=base_url()?>pembelajaran/kenaikankelas/loadkelasakhir/' + $('#mp_id').val() + '/' + $('#tahunajaranakhir').val());
		}		
	});	
