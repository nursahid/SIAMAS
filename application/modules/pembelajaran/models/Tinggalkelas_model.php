...
class Tinggalkelas_model extends Base_Model {
	...
	//list data kelas 8,9, 10, 11
	function get_kelas_tinggalkelas() {
		$arr = $this->db->query("SELECT * FROM kelas WHERE id_tahun=((SELECT id FROM seting_tahun_ajaran WHERE status='Y') - 1) AND (id_tingkat='1' OR id_tingkat='2' OR id_tingkat='3' OR id_tingkat='4' OR id_tingkat='5')");
	...
	function list_siswa_perkelas($idkelas, $idtahun) {
		$idnow = $idtahun + 1;
		$arr = $this->db->query("SELECT `siswa_kelas`.`id_siswa`, `siswa`.`id`, `siswa`.`nama` FROM (`siswa_kelas`) JOIN `siswa` ON `siswa`.`id`=`siswa_kelas`.`id_siswa` WHERE `id_kelas` = '".$idkelas."' AND `id_tahun` = '".$idtahun."' AND `siswa_kelas`.`id_siswa` NOT IN (SELECT `id_siswa` FROM siswa_kelas WHERE id_tahun='".$idnow."')");
