...
class Smsgateway extends MY_Controller
{
    ...
    public function index()
    {
		...
		$template_data['subtitle'] = 'Konfigurasi Awal';
        $template_data['crumb'] = ['Seting SMS' => 'seting/smsgateway',];
		
        $this->layout->set_wrapper('smsgateway', $data); 
    ...
    public function simpan()
    {
		...
		$this->form_validation->set_rules('namamodem', 'Nama Modem', 'required');
		$this->form_validation->set_rules('portmodem', 'Port Modem', 'required');
		$this->form_validation->set_rules('koneksimodem', 'Koneksi Modem', 'required');
		$this->form_validation->set_rules('lokasiserver', 'Lokasi Server Gammu', 'required');
		...
		if($this->form_validation->run() == FALSE) {
			...
			//Update database 
			$data = array(
					'gammu_namamodem' 	=> $this->input->post('namamodem',TRUE),
					'gammu_port' 	  	=> $this->input->post('portmodem',TRUE),
					'gammu_connection' 	=> $this->input->post('koneksimodem',TRUE),
					'lokasi_server' 	=> $this->input->post('lokasiserver',TRUE),
			);
			$this->setting_model->save($data);
			//tulis file config
			$this->write('gammurc',$_POST['portmodem'],$_POST['koneksimodem']);
    ...
    public function read($file)
    {
		...
		$readconfig = read_file(''.$setting['lokasi_server'].'/bin/'.$file.'');
		
		if ( ! read_file(''.$setting['lokasi_server'].'/bin/'.$file.'')) {
			$data['message'] = 'Unable to read the file pada '.$setting['lokasi_server'].'/bin/';
		} else {
			$data['message'] = '';
			$data['fileconfig'] = $file;
		}
		
		$data['setting'] 	= $setting;
		$data['configgammu']= $readconfig;
			
		$template_data['title'] = 'Seting dan Konfigurasi';
		$template_data['subtitle'] = 'Konfigurasi Awal';
		$template_data['crumb'] = ['Seting SMS' => 'seting/smsgateway',];
			
		$this->layout->set_wrapper('read_gammuconfig', $data);
		$this->layout->setCacheAssets();
		$this->layout->render('admin', $template_data);		

	}
	//tulis
    public function write($file,$port,$connection)
    {
		$setting = $this->settings->get();
		
		$data = "; This is a sample ~/.gammurc file.
...
";
		...
		if ( ! write_file(''.$setting['lokasi_server'].'/bin/'.$file.'', $data))
		{
			echo json_encode(array('status'=>0, 'pesan' => '<div class="text-red"><i class="fa fa-info"></i> Unable to write the file pada '.$setting['lokasi_server'].'/bin/ </div>'));
		}
		else
		{
			echo json_encode(array('status'=>1, 'pesan' => '<div class="text-red"><i class="fa fa-check"></i> Data Berhasil disimpan</div>'));
...
/* Location: ./application/seting/controllers/Smsgateway.php */
