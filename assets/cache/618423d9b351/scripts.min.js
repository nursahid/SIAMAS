...
{if(list.dragEl){e.preventDefault();list.dragStop(e.touches?e.touches[0]:e)}};if(hasTouch){list.el[0].addEventListener('touchstart',onStartEvent,!1);window.addEventListener('touchmove',onMoveEvent,!1);window.addEventListener('touchend',onEndEvent,!1);window.addEventListener('touchcancel',onEndEvent,!1)}
...
{this.mouse={offsetX:0,offsetY:0,startX:0,startY:0,lastX:0,lastY:0,nowX:0,nowY:0,distX:0,distY:0,dirAx:0,dirX:0,dirY:0,lastDirX:0,lastDirY:0,distAxX:0,distAxY:0};this.isTouch=!1;this.moving=!1;this.dragEl=null;this.dragRootEl=null;this.dragDepth=0;this.hasNewRoot=!1;this.pointEl=null},expandItem:function(li)
...
{var list,parent,prev,next,depth,opt=this.options,mouse=this.mouse;this.dragEl.css({'left':e.pageX-mouse.offsetX,'top':e.pageY-mouse.offsetY});mouse.lastX=mouse.nowX;mouse.lastY=mouse.nowY;mouse.nowX=e.pageX;mouse.nowY=e.pageY;mouse.distX=mouse.nowX-mouse.lastX;mouse.distY=mouse.nowY-mouse.lastY;mouse.lastDirX=mouse.dirX;mouse.lastDirY=mouse.dirY;mouse.dirX=mouse.distX===0?0:mouse.distX>0?1:-1;mouse.dirY=mouse.distY===0?0:mouse.distY>0?1:-1;var newAx=Math.abs(mouse.distX)>Math.abs(mouse.distY)?1:0;if(!mouse.moving){mouse.dirAx=newAx;mouse.moving=!0;return}
...
{var plugin=$(this).data("nestable");if(!plugin){$(this).data("nestable",new Plugin(this,params));$(this).data("nestable-id",new Date().getTime())}else{if(typeof params==='string'&&typeof plugin[params]==='function'){retval=plugin[params]()}}});return retval||lists}})(window.jQuery||window.Zepto,window,document);$(function(){var url='';if($.inArray('table',segment)===-1){url=segment[1]}else{url=segment[1];if(segment[2]!==''){url+='/'+segment[2]}}
